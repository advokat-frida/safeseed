...
export type ReservedSpec =
  ...
  | {
      ...
      values: readonly { source: string; digest: string }[];
    }
  | {
      kind: "marketingUrl";
      baseUrl: string;
      params: readonly { name: string; tokenPrefix: string }[];
    }
...
const CLAIM_HASHED_EMAIL =
  "SHA-256 digest of a catalog-listed email under an RFC 2606 reserved domain. The input reservation is the assurance source; the digest itself is neither reserved nor visibly distinguishable from a hash of real data. SafeSeed accepts only the published digest allowlist.";
const CLAIM_HASHED_PHONE =
  "SHA-256 digest of a catalog-listed NANPA fictitious phone number normalized to E.164. The authority policy on the input is the assurance source; the digest itself is neither reserved nor visibly distinguishable from a hash of real data. SafeSeed accepts only the published digest allowlist.";
const CLAIM_MARKETING_URL =
  "Canonical HTTPS URL on an RFC 2606 reserved host with deliberately obvious TEST_ attribution parameters. The host reservation and exact token pattern support the claim; an arbitrary URL on a reserved host is not accepted.";
  ...
  switch (r.kind) {
    ...
    case "ukDramaPhoneBlock": {
      ...
      if (digits.length === 12 && digits.startsWith("44")) {
        ...
        if (!value.startsWith("+44")) return false;
        national = `0${digits.slice(2)}`;
      }
      if (national.length !== 11 || !national.startsWith(r.nationalPrefix)) return false;
    ...
    case "marketingUrl": {
      ...
      if (params.length !== r.params.length) return false;
