...
## Current release boundary (2026-08-18)

- npm currently serves `safeseed@0.2.1`.
- Git contains `v0.2.0` and `v0.2.1` tags but no GitHub Releases.
- The `0.3.0` candidate exists only in the local working tree. It has not been committed, pushed,
...
Record evidence in [`docs/release-readiness-v0.3.0.md`](docs/release-readiness-v0.3.0.md) and keep
...
- 88 unit tests, the three-case CLI boundary contract, and the five-case local Action contract pass.
...
- `package.json`, `package-lock.json`, `SAFESEED_VERSION`, the example run record, README Action tag,
  and top changelog entry all say `0.3.0`.
...
1. Draft a GitHub Release titled `SafeSeed 0.3.0`, targeting the exact reviewed commit on `main`
   and tag `v0.3.0`.
2. Use the `0.3.0` changelog entry as release notes. Attach every intended asset before
...
```powershell
npm.cmd view safeseed@0.3.0 version dist.integrity dist.shasum --json
npx.cmd --yes safeseed@0.3.0 version
```

The expected CLI line begins `safeseed 0.3.0`. Confirm npm shows provenance and that the package,
